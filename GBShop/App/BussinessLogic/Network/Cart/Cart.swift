...
extension Cart: CartRequestFactory {
    ...
    func addCart(
        ...
        let requestModel = AddCart(baseUrl: baseUrl, item: item, price: price, amount: amount
)
        self.request(
    ...
    func makeCart(
        ...
        amount: Int,
        bill: Int,
        completionHandler: @escaping (AFDataResponse<CartBalanse>
        ) -> Void) {
        let requestModel = CartBalanse(
            item: item,
            price: price,
            amount: amount,
            bill: bill
...
extension Cart {
    ...
    struct AddCart: RequestRouter {
        ...
        let path: String = "dummyAddCart.json"
        let item: String
        let price: Int
        let amount: Int
    ...
    struct RemoveCart: RequestRouter {
        let baseUrl: URL
        let method: HTTPMethod = .get
        let path: String = "dummyRemoveCart.json"
        let item: String
        let amount: Int
        var parameters: Parameters? {
            return [
